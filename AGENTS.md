...
- **Locks**: screws fully banned at every layer (player RPC, `initiateScrew`/`selectScrewee`/`startScrewTimer` server-side, `hostInitiateScrew`, host button; enabling coop clears a live screw). JACK hidden/coerced (`JACK_MULTIPLIER_OPTIONS` = 1/1.5/2/2.5/3, clamped in UI + `setHostSetting`), re-buzz forced off, options locked 4+, shared-team scoring forced to alliance, fibbage + disordat + quixort blocked in UI **and** at `fibbage-lie`/`fibbage-vote` / `disordat-answer`/`disordat-claim` / `quixort-place` RPC level. Mode/mode-toggle only from buttons/text; coop can't enable mid-fibbage/disordat/quixort.
...
- `npm run test:harness` — stubbed PlayroomKit+DOM driving real handlers: buzz/ruling math, edits, bingo/disordat/fibbage/quixort gates, screw ban + victim-hijack rejection, non-producer `producer-action` rejection, roster accounting, roulette freeze, reset null-invariant, room-code spotlight broadcast, reset-all scores, credits team grouping, rendered HTML for host/player/audience views, no-render-warning check. A dedicated `prod1` producer fixture drives all `producer-action` calls (sender auth); player RPCs stay on `dev*` fixtures. Extend it before trusting multi-step logic by reasoning alone — stale-`round` overwrites and signature staleness both survived reasoning and died in the harness.
