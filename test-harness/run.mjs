...
if (COOP) {
  ...
  await pk._store.rpc["cohost-action"]({ fn: "resetScrews", args: [] }, coh);
  await pk._store.rpc["cohost-action"]({ fn: "closeScrewMode", args: [] }, coh);
}
...
await pk._store.rpc["cohost-action"]({ fn: "adjustPlayerScore", args: ["solo1", 500] }, coh);
...
await pk._store.rpc["cohost-action"]({ fn: "adjustPlayerScore", args: ["solo1", NaN] }, coh);
check("NaN adjust no-op", JSON.stringify(S().scores) === nanAdjBefore, "scores changed on NaN");
await pk._store.rpc["cohost-action"]({ fn: "adjustPlayerScore", args: ["solo1", 0] }, coh);
check("zero adjust no-op", (S().scores?.solo1 || 0) === soloBefore + 500, JSON.stringify(S().scores?.solo1));
const impAdj = await pk._store.rpc["cohost-action"]({ fn: "adjustPlayerScore", args: ["solo1", 100] }, impostor);
check("impostor adjust rejected", impAdj?.ok === false, JSON.stringify(impAdj));
await pk._store.rpc["cohost-action"]({ fn: "resetPlayerScore", args: ["solo1"] }, coh);
check("reset single score zeroes", (S().scores?.solo1 || 0) === 0, JSON.stringify(S().scores?.solo1));
await pk._store.rpc["cohost-action"]({ fn: "setCustomPlayerName", args: ["solo1", "Renamed"] }, coh);
...
await pk._store.rpc["cohost-action"]({ fn: "setCustomPlayerName", args: ["solo1", ""] }, coh);
check("rename clear", S().customNames?.solo1 === undefined, JSON.stringify(S().customNames));
await pk._store.rpc["cohost-action"]({ fn: "setPlayerScrewBlocked", args: ["solo1", true] }, coh);
...
await pk._store.rpc["cohost-action"]({ fn: "setPlayerScrewBlocked", args: ["solo1", false] }, coh);
...
await pk._store.rpc["cohost-action"]({ fn: "refundPlayerScrew", args: ["solo1"] }, coh);
check("screw refunded", !(S().round?.screwsUsedBy || []).includes("solo1"), JSON.stringify(S().round?.screwsUsedBy));
await pk._store.rpc["cohost-action"]({ fn: "kickPlayer", args: ["solo1"] }, coh);
...
// team-mode open requires every non-controller/cohost player assigned
for (const pid of Object.keys(pk._store.participants)) {
  if (pid === "host1" || pid === "coh1") continue;
  await pk._store.rpc["cohost-action"]({ fn: "setPlayerTeam", args: [pid, "red"] }, coh);
}
await pk._store.rpc["cohost-action"]({ fn: "openBuzzers", args: [] }, coh);
